-- Buckets de Storage para las fotos de empleados, de artículos, de los
-- vehículos que entran al taller y de los adjuntos del chat del equipo.
...
create policy storage_leer_publico on storage.objects for select to public
  using (bucket_id in ('empleados', 'productos', 'vehiculos', 'chat'));
...
create policy storage_subir on storage.objects for insert to authenticated
  with check (bucket_id in ('empleados', 'productos', 'vehiculos', 'chat'));
...
-- Verificación. Tienen que salir los 4 buckets en public=true y 4 políticas.
...
select id, public from storage.buckets where id in ('empleados', 'productos', 'vehiculos', 'chat');
